...
// ── Turnos de demo: se calculan relativos a "ahora" cada vez que carga ──
// la app, en vez de usar una fecha fija, para que "Mi Turno" muestre un
// tiempo transcurrido/restante creíble sin importar cuándo se abra la demo.
function _horaInicioHace(horas, minutos = 0) {
  const d = new Date(Date.now() - (horas * 60 + minutos) * 60000);
  const yyyy = d.getFullYear();
  const mm2 = String(d.getMonth() + 1).padStart(2, "0");
  const dd = String(d.getDate()).padStart(2, "0");
  const hh = String(d.getHours()).padStart(2, "0");
  const mm = String(d.getMinutes()).padStart(2, "0");
  return { fecha: `${yyyy}-${mm2}-${dd}`, hora_inicio: `${hh}:${mm}` };
}
...
export const MockDB = {
  ...
  turnos_activos: [
    { id: 1, empleado_id: 5, ..._horaInicioHace(3, 20), hora_fin: null, estado: "activo" }, // Ana Torres — empezó hace 3h20
    { id: 2, empleado_id: 7, ..._horaInicioHace(4, 45), hora_fin: null, estado: "activo" }, // Miguel Flores — empezó hace 4h45
  ],
