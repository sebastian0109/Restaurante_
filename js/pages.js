...
// API expone: getPedidos, createPedido, ejecutarAccionPedido, getMesas,
...
//  se seleccionan en pointerup: como wrap.setPointerCapture()
//  retarget el click nativo hacia `wrap`, el toque simple se
//  detecta manualmente (sin arrastre) y se invoca el onclick
//  original de la mesa real bajo el dedo/cursor.
// ══════════════════════════════════════════════════════
const MapZoom = {
  _ctx: null,
  ...
  init(wrapId, innerId) {
    ...
    wrap.addEventListener("pointerdown", (e) => {
      ...
      if (ctx.pointers.size === 1) {
        ...
        ctx.tapStart = { x: e.clientX, y: e.clientY, target: e.target };
    ...
    wrap.addEventListener("pointerup", (e) => {
      // FIX: wrap.setPointerCapture() (en pointerdown) retarget los eventos
      // del puntero —incluido el click sintético— hacia `wrap`, por lo que
      // el onclick de la mesa nunca llega a dispararse solo. Si el toque no
      // se desplazó (no fue un pan/pinch), se invoca manualmente el onclick
      // original del elemento real bajo el dedo/cursor.
      if (ctx.pointers.size === 1 && ctx.tapStart) {
        const dist = Math.hypot(e.clientX - ctx.tapStart.x, e.clientY - ctx.tapStart.y);
        if (dist < 10) {
          const mesaEl = ctx.tapStart.target.closest && ctx.tapStart.target.closest(".mesa-grupo");
          if (mesaEl && typeof mesaEl.onclick === "function") mesaEl.onclick(e);
        }
      }
...
async function renderNuevoPedido(cid, usuario) {
  ...
  const C = {
    ...
    con_pedido:     { bg:"#0ea5e9", txt:"#fff",    glow:"rgba(14,165,233,0.4)", badge:"#e0f2fe", badgeTxt:"#075985" },
    pendiente_pago: { bg:"#64748b", txt:"#fff",    glow:"rgba(100,116,139,0.4)",badge:"#e2e8f0", badgeTxt:"#334155" },
    liberable:      { bg:"#8b5cf6", txt:"#fff",    glow:"rgba(139,92,246,0.4)", badge:"#ede9fe", badgeTxt:"#5b21b6" },
  };
...
async function renderTurno(cid, usuario) {
  ...
  // Duración normal de jornada (misma referencia que usa el scheduler
  // de notificaciones para avisos de 30/15 min y horas extra: 8 horas).
  const DURACION_NORMAL_MIN = 480;
      ...
      ${miTurno ? `
        <div class="card border-0 shadow-sm mb-4">
          <div class="card-body py-4">
            ...
            <p class="text-muted mb-3">Fecha: <strong>${miTurno.fecha}</strong></p>
            <hr class="my-3">
            <div id="turno-restante"></div>
          </div>
        </div>
  ...
  if (miTurno) {
    ...
    const actualizarRestante = () => {
      const contenedor = document.getElementById("turno-restante");
      // Si ya navegamos a otra página, detener el intervalo.
      if (!contenedor) { clearInterval(intervalId); return; }

      const inicio = new Date(`${miTurno.fecha}T${miTurno.hora_inicio}:00`);
      const minutosTranscurridos = (Date.now() - inicio.getTime()) / 60000;
      const minutosRestantes = DURACION_NORMAL_MIN - minutosTranscurridos;

      if (minutosRestantes > 0) {
        const h = Math.floor(minutosRestantes / 60);
        const m = Math.round(minutosRestantes % 60);
        contenedor.innerHTML = `
          <p class="text-muted mb-1 small text-uppercase fw-semibold">Tiempo restante de turno</p>
          <h3 class="fw-bold ${minutosRestantes <= 30 ? 'text-warning' : 'text-primary'} mb-0">
            <i class="bi bi-hourglass-split me-1"></i>${h}h ${m}min
          </h3>`;
      } else {
        const h = Math.floor(-minutosRestantes / 60);
        const m = Math.round(-minutosRestantes % 60);
        contenedor.innerHTML = `
          <p class="text-muted mb-1 small text-uppercase fw-semibold">Jornada normal completada</p>
          <h3 class="fw-bold text-danger mb-0">
            <i class="bi bi-exclamation-triangle-fill me-1"></i>+${h}h ${m}min extra
          </h3>`;
      }
    };
    actualizarRestante();
    intervalId = setInterval(actualizarRestante, 30000);
  }
}
...
async function renderMesas(cid, usuario) {
  ...
  // Roles habilitados según TRANSICIONES_MESA (businessLogic.js):
  //   reservar / confirmar_llegada / ocupar_sin_reserva → portero, administrador, jefe_meseros
  //   liberar_mesa                                      → portero, administrador
  const puedeGestionarMesa = ["portero", "administrador", "jefe_meseros"].includes(usuario.rol);
  const puedeLiberar       = ["portero", "administrador"].includes(usuario.rol);
  const esMesero  = usuario.rol === "mesero";
  ...
  const tarjetas = mesas.map(mesa => {
    ...
    if (puedeGestionarMesa) {
      ...
      if (mesa.estado === "liberable" && puedeLiberar)
...
async function renderSolicitudesIngredientes(cid, usuario) {
  ...
  const solicitudes = res.data;
  const puedeAprobar = ["administrador", "jefe_cocina", "dueno"].includes(usuario.rol);
...
export const PageActions = {
  ...
  async cambiarEstadoPedido(id, accion) {
    const usuario = AuthService.getUsuario();
    const res = await ejecutarAccionPedido(id, accion, usuario.rol, usuario.id);
    if (res.status === 200) {
      Toast.success(`Pedido #${id} → ${accion.replace("_", " ")}`);
  ...
  async verMesaDetalle(mesaId) {
    ...
    // Nota: este modal es informativo para el mesero (ver pedido de una mesa
    // ocupada mientras navega el plano). Cambiar el estado de la mesa
    // (ocupar/liberar/confirmar reserva) es responsabilidad de portero/admin
    // y se hace desde la pantalla "Mesas", no desde aquí.
    Modal.show({
  ...
  async rechazarIngrediente(id) {
    const res = await API.updateSolicitudIngrediente(id, { estado: "rechazada" });
    if (res.status === 200) { Toast.warning("Solicitud rechazada"); App.navigate(App.currentPage); }
  },
